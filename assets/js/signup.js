// Keep API requests on the current HTTPS origin in production and staging.
const API_BASE_URL = new URL("/drmapinew/", window.location.origin).href;
        ...
        if (data.responseCode === 200 && result.respCode === 200) {
            ...
            if (typeof window.gtag === "function") {
                window.gtag("event", "otp_requested", {
                    method: isResend ? "resend" : "initial"
                });
            }
...
async function createAccount() {
    ...
    try {
        const data = await postApi(CREATE_ACCOUNT_PATH, requestBody);
        const result = data.payload || {};
        if (data.responseCode === 200 && result.respCode === 200) {
            if (typeof window.gtag === "function") {
